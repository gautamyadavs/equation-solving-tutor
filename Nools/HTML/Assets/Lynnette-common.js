...
function showStepDiagrams(params) {
  ...
  let originalEquation = regenerateEquation(...params[0]),
...
function regenerateEquation(modifiedEquation, operation, operand) {
  let [leftSide, rightSide] = algebraParser.algGetOperands(algebraParser.algParse(modifiedEquation)),
      operator = operation == 'subtraction' ? 'PLUS' : 'TIMES',
      operandTree = algebraParser.algParse(operand),
      newLeftSide = newExpression(operator, leftSide, operandTree),
      newRightSide = newExpression(operator, rightSide, operandTree);

  return algebraParser.algSimplify(newExpression('EQUAL', newLeftSide, newRightSide));
}
